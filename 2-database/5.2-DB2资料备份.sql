...
--ROW_NUMBER：给查询结果集编号；
--OVER：并不是一个函数，而是一个表达式，
--它的作用是定义一个作用域（或者可以说是结果集），
...
请注意序号和部门序号之间的区别，我们在查询部门序号的时候，
在OVER表达式中多了两个子句，分别是PARTITION BY 和ORDER BY。
它们有什么作用呢？在介绍它们的作用之前，我们先来回顾一下OVER的作用，还记得吗？

OVER是一个表达式，它的作用是定义一个作用域（或者可以说是结果集）
，OVER前面的函数只对OVER定义的结果集起作用。

ORDER BY的作用大家应该非常熟悉，用来对结果集排序。
PARTITION BY的作用其实也很简单，和GROUP BY 的作用相同，用来对结果集分组。

 
  到此为止，大家应该对OLAP函数的套路有一定的了解和体会了吧。
  大家看一下上面SQL的结果集，发现王五的工资是null，
  当我们按工资排序时，null被放到最后，我们想把null放在前边该怎么办呢？
...
请注意ROW_NUMBER和RANK之间的区别，RANK是等级，排名的意思，
李四和李白的工资都是5000，他们并列排名第二。
张三和王蓝的工资都是4000，怎么RANK函数的排名是第四，
而DENSE_RANK的排名是第三呢？这正是这两个函数之间的区别。
